...
func main() {
	...
	subF, err := redisstream.NewSubscriber(redisstream.SubscriberConfig{
		Client: rdb,
	}, logger)
	...
	router.AddNoPublisherHandler("temp-fahrenheit",
		"temperature-fahrenheit", subF, func(msg *message.Message) error {
			tempFahrenheit := string(msg.Payload)
